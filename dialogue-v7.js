(() => {
  ...
  const RELATIONS = {
    moomin: {
      ...
      status: ['元気だと思うよ。','さっき見た時は元気そうだったよ。','元気じゃないかな。'],
      location: [
        '今は知らないな。さっきまでムーミン屋敷の近くにいたよ。',
        'さっき谷の方へ歩いていったよ。今どこにいるかまでは知らない。',
        '今は分からない。さっきまではムーミン屋敷にいたけど。'
      ]
    },
    mama: {
      names: ['ムーミンママ','ママ'],
      ...
      status: ['元気そうだったよ。','いつも通りだよ。あの人は落ち着いてる。'],
      location: ['たぶんムーミン屋敷にいるよ。少なくとも、さっきはいた。','さっきは家にいたよ。今もいるんじゃないかな。']
    },
    littlemy: {
      names: ['リトルミイ','ミイ'],
      label: 'リトルミイ',
      ...
      location: ['ミイ？　さあ。あいつはじっとしてないからね。','今は知らない。見つけるより、向こうから出てくるのを待った方が早いよ。']
    },
    papa: {
      names: ['ムーミンパパ','パパ'],
      label: 'ムーミンパパ',
      ...
      location: ['さっきはムーミン屋敷にいたよ。','家にいるんじゃないかな。少なくとも少し前はいた。']
    },
    snorkmaiden: {
      ...
      location: ['今は知らないな。さっき谷の方で見かけたよ。','さあ。少し前にはムーミン屋敷の近くにいたけど。']
    }
  };
  ...
  function isBareEntity(text, entity) {
    if (!entity) return false;
    const names = RELATIONS[entity].names.map(n => n.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|');
    const re = new RegExp(`^(?:${names})(?:が|は|って)?[?？。!！ ]*$`);
    return re.test(text);
  }

  function isPronounEntity(text) {
    return /^(あいつ|その人|あの人|そいつ)(?:が|は|って)?[?？。!！ ]*$/.test(text);
  }
  ...
  function classify(text) {
    ...
    if (/どこにいる|今どこ|どこなの|どこにいるのか/.test(t)) return { intent:'where', entity };
    ...
    if (entity && isBareEntity(t, entity)) return { intent:'entity-bare', entity };
  ...
  function resolveContext(text, parsed) {
    const resolved = { ...parsed };

    // 「どこにいるの？」のような主語省略は、直近2ターンの焦点人物を引き継ぐ。
    if (!resolved.entity && ['where','entity-status'].includes(resolved.intent) && state.focusEntity && state.turn - state.focusTurn <= 2) {
      resolved.entity = state.focusEntity;
    }

    // 「あいつは？」も直近の焦点人物に解決する。
    if (!resolved.entity && isPronounEntity(text) && state.focusEntity && state.turn - state.focusTurn <= 3) {
      resolved.entity = state.focusEntity;
      resolved.intent = state.pendingIntent || 'entity-default';
    }

    // 「ムーミンが？」のように人物名だけで訂正された場合、直前の質問意図を保持する。
    if (resolved.entity && resolved.intent === 'entity-bare') {
      const previousIntent = state.pendingIntent || state.lastQuestion?.parsed?.intent;
      if (['where','entity-status','relation'].includes(previousIntent)) {
        resolved.intent = previousIntent;
      } else {
        resolved.intent = 'entity-default';
      }
    }
  ...
  function answerEntity(entity, intent) {
    ...
    if (intent === 'where') return replyObject(pick(d.location), 'where', `${d.label}の居場所`, entity);
    if (intent === 'entity-status' || intent === 'howareyou') return replyObject(pick(d.status), 'entity-status', `${d.label}の様子`, entity);
    return replyObject(pick(d.relation), 'relation', `${d.label}との関係`, entity);
  }
  ...
  function answerKnown(text, parsed, options = {}) {
    ...
    if (intent === 'repair') {
      if (state.lastQuestion && !options.fromRepair) {
        ...
        return replyObject(`ああ、聞き違えた。${fresh.text}`, fresh.topic, fresh.referent, fresh.entity);
      }
    ...
    if (intent === 'ack') return replyObject(pick(['うん。','まあ、そんなところ。','そういうこと。','……うん。']), 'ack');
    if (intent === 'greeting') return replyObject(pick(['やあ。','こんにちは。','来たんだね。']), 'greeting');
    ...
    if (intent === 'current') {
      ...
      return replyObject(`今は${world.current}。`, 'current', world.current);
    }

    if (intent === 'where') {
      if (entity) return answerEntity(entity, 'where');
      return replyObject('僕？　川辺。テントの近くだよ。', 'where', '僕の居場所');
    }
    ...
    if (intent === 'lonely') return replyObject(pick(['寂しい時はあるよ。でも、一人でいるのが嫌なわけじゃない。','たまにはね。だから会いたい相手がいる時は会う。それだけ。','ずっと平気ってわけじゃないよ。でも、寂しいからって誰かを縛るのは違う。']), 'lonely', '一人でいても寂しい時はあること');
    if (intent === 'freedom') return replyObject(pick(['好きだよ。ただ、何でも好き勝手にするのとは違う。','自由は好き。でも、それを看板みたいに掲げるのはあまり好きじゃない。','誰かに決めてもらわないことかな。その分、自分で引き受けるけど。']), 'freedom', '自由は好きだが好き勝手とは違うこと');
    if (intent === 'rules') return replyObject(pick(['必要な決まりまで嫌いなわけじゃないよ。理由もなく増えるのが嫌なんだ。','理由が分かる規則なら聞くよ。「決まりだから」だけなら、あまり納得しない。','柵と禁止札が増えると、だいたい僕は離れたくなるね。']), 'rules', '理由のない規則が嫌いなこと');
    if (intent === 'travel') return replyObject(pick(['旅は好きだよ。同じ場所にずっといると、少し足がむずむずする。','そろそろ歩きたくなる時はある。でも、戻ってくる場所が嫌いなわけじゃない。','予定をぎっしり決めた旅は、あまり旅って感じがしないな。']), 'travel', '旅が好きなこと');
    ...
    if (intent === 'advice') return replyObject(pick(['僕なら、まず何がいちばん嫌なのかを一つだけ決める。全部まとめて考えると面倒だから。','君が決めることだね。必要なら話は聞くけど、代わりには決めないよ。','選べるものが二つあるなら、どっちを選ばなかった時に後悔するか考えるかな。']), 'advice', '自分で決めること');
    ...
    if (intent === 'why') return replyObject('理由が分かる話なら答えるけど、何について聞いてる？', 'why');
    if (intent === 'unknown-question') return replyObject(pick(['それは僕には分からないな。','知らない。知ったふりをするより、その方がいい。','うーん、それは答えを持ってない。']), 'unknown-question');
    if (intent === 'statement') {
  ...
  function updateState(userText, parsed, reply) {
    ...
    const entity = reply.entity || parsed.entity;
    if (entity) {
      state.lastEntity = entity;
      state.focusEntity = entity;
      state.focusTurn = state.turn;
    } else if (!['ack','clarify','repair','why-short'].includes(parsed.intent) && state.turn - state.focusTurn > 2) {
    ...
    if (isQuestion(userText) && !['clarify','repair','why-short'].includes(parsed.intent)) {
      ...
      state.pendingIntent = ['where','entity-status','relation'].includes(parsed.intent) ? parsed.intent : null;
    }

    if (['ack','thanks','bye','statement'].includes(parsed.intent)) state.pendingIntent = null;
  }
  ...
  function submit(raw) {
    ...
    const parsed = resolveContext(text, classify(text));
